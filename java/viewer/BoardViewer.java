...
public class BoardViewer {
    ...
    public void setCommentView(CommentViewer commentViewer) {
    ...
    private void printOne(int id) {
        ...
        if (boardDTO.getWriterId() == login.getId()) {
            message = "1. 수정 2. 삭제 3. 댓글보기 4. 뒤로가기";
            userChoice = ScannerUtil.nextInt(SCANNER, message, 1, 4);
        } else {
            message = "3. 댓글보기 4. 뒤로 가기";
            userChoice = ScannerUtil.nextInt(SCANNER, message, 3, 4);
        }
        ...
        if (userChoice == 1) {
            ...
            commentViewer.printCommentList(id);
        } else if (userChoice == 4) {
            printList();
        }
    }
    ...
    public void userUpdate(UserDTO u) {  // 보드 유저 실시간 업데이트
        ArrayList<BoardDTO> list = boardController.selectAllWriteId(u.getId());
        for (BoardDTO b : list) {
            b.setWriterNickname(u.getNickname());
            boardController.update(b);
        }
    }

    public void userDelete(UserDTO u) {  // 보드 유저 실시간 삭제
        ArrayList<BoardDTO> list = boardController.selectAllWriteId(u.getId());
        for (BoardDTO b : list) {
            boardController.delete(b.getId());
        }
    }
     */
