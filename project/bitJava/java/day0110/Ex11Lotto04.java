...
public class Ex11Lotto04 {
    ...
    public final static int SIZE_MAX = 45;
    public final static int SIZE_MIN = 1;
    public final static int LOTTO_SIZE = 6;
    public static int[] userNumber = new int[LOTTO_SIZE];
    public static int[] comNumber = new int[LOTTO_SIZE];
    public static void main(String[] args) {

        String message = "몇 게임할래?";
        int gameCount = ScannerUtil.nextInt(SCANNER, message);
    ...
    public static void autoNumber(){
        ...
        for (int i =0; i< LOTTO_SIZE;){
            int temp = random.nextInt(SIZE_MAX)+SIZE_MIN;
            numberSwitch = true;

            for(int j=0 ; j< LOTTO_SIZE;j++){
                if(userNumber[i] == temp){
                    numberSwitch = false;
                }
    ...
    public static void dirNumber(){
        boolean numberSwitch = false;
        for(int i=0; i< LOTTO_SIZE;){
            String message = i+ "번째 숫자를 입력해주세요.";
            numberSwitch = true;
            int temp = ScannerUtil.nextInt(SCANNER, message, SIZE_MAX, SIZE_MIN);

            for(int j=0; j<i; j++){
                if (userNumber[j] == temp){
                    numberSwitch = false;
                }
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        Ex10Lotto3.setAutoNumbers(computerNumbers);
        Ex10Lotto3.sort(computerNumbers);
    ...
    public static void setNumbers(int[][] arrays){
        for(int i =0; i<arrays.length; i++){
            ...
            if(userChoice ==1){
                Ex10Lotto3.setAutoNumbers(arrays[i]);
            }else{
                Ex10Lotto3.setManualNumbers(arrays[i]);
            }
            Ex10Lotto3.sort(arrays[i]);
        }
    ...
    public static int countSame(int[] computerNumbers, int[] userNumbers){
        ...
        for(int i=0; i< computerNumbers.length; i++){
            if(Ex10Lotto3.contains(userNumbers, computerNumbers[i])){
                count++;
            }
        }
